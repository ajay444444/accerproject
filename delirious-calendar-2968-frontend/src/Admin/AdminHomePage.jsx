...
export const AdminHomePage = () => {
  ...
  let handleBlockUSer = (id)=>{
   ...
   .then((res)=>{
    ...
    let getReviewsData = ()=>{
      axios.get(`http://localhost:8080/review/get`)
        .then((res)=>{
    ...
    const DeleteReviewsData = (id)=>{
      let bodyofreq = {
        reviewID : id
      }
      axios.delete(`http://localhost:8080/review/delete`,{headers:{
  ...
  return (
    <div  style={{backgroundColor:"#ffffff", paddingTop:"70px"}}>
      ...
      <Flex   alignItems={"center"} height="100px" bg="#dccbd5">
        ...
        <Button _hover={{borderRadius:"25px", bg:"#e3e8e6"}} borderRadius={"none"} outline={"none"} bg="#dccbd5" onClick={handleReviewsData}>Reviews</Button>
...
{
        ReviewsState && <div style={{marginTop:"40px"}}>
                    <Box display={"flex"} justifyContent={"center"} alignItems={"center"} gap="18px">
                    <FaDog style={{height:"50px",width:"50px"}}/>
                    <h1 style={{fontSize:"24px", fontWeight:"bold", marginTop:"15px"}}> Reviews</h1>
                    </Box>
                    <Box  display={"grid"} gridTemplateColumns={"1fr 1fr "} gap="20px" mt="45px"  >

            {
                Reviews.map((ele)=>(
                    <div>
                      <ReviewCard {...ele} DeleteReviewsData={DeleteReviewsData}/>
                    </div>
                ))
            }
                    </Box>
